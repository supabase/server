...
The context is stored in `event.context.supabaseContext` and contains the same `SupabaseContext` fields as the main `withSupabase` wrapper: `supabase`, `supabaseAdmin`, `userClaims`, `claims`, and `authType`.
...
app.get('/todos', withSupabase({ allow: 'user' }), async (event) => {
  const { supabase } = event.context.supabaseContext
  const { data } = await supabase.from('todos').select()
  return data
})
...
app.post('/admin/sync', withSupabase({ allow: 'secret' }), async (event) => {
...
app.get(
  '/reports',
  withSupabase({ allow: ['user', 'secret'] }),
  async (event) => {
    const { authType } = event.context.supabaseContext
    return { authType }
  },
)
...
app.post('/webhook', withSupabase({ allow: 'secret' }), async (event) => {
  const { supabaseAdmin } = event.context.supabaseContext
  // ...
})
```
