...
For routes that compose with a [gate](../../src/core/gates/README.md), call `withSupabase` with two args. That form is the base `withSupabase` from `@supabase/server` — it returns a Web Fetch handler, which you mount on an H3 route:
