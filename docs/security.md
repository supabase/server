...
4. If `audience` is configured in `env` overrides, the token's `aud` claim must match
5. If `issuer` is configured in `env` overrides, the token's `iss` claim must match
...
**Audience and issuer validation.** In setups where multiple services share the same signing keys, a JWT minted by one service could be accepted by another. Passing `audience` and `issuer` via configuration overrides (e.g. `env: { audience: 'my-service', issuer: 'https://...' }`) prevents this by rejecting tokens that weren't issued for your specific service. Both are optional for backward compatibility but recommended in multi-service deployments.
