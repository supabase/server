...
describe('defineGate — exposes key + run metadata', () => {
  it('attaches the literal key to the returned factory', () => {
    const withGreeting = defineGate<
      'greeting',
      undefined,
      Record<never, never>,
    ...
    >({
      key: 'greeting',
      run: () => async () => ({ greeting: { hello: 'world' } }),
    })

    expect(withGreeting.key).toBe('greeting')
  })

  it('exposes run for direct invocation by guard-shaped adapters', async () => {
    const withGreeting = defineGate<
      'greeting',
      { who: string },
      Record<never, never>,
    ...
    // Adapters that can't use the fetch-handler wrapper form (e.g. NestJS
    // guards) drive the check directly via run(config)(req, upstream).
    const check = withGreeting.run({ who: 'world' })
    const result = await check(new Request('http://localhost/'), {})

    expect(result).toEqual({ greeting: { hello: 'world' } })
  })

  it('preserves run short-circuit semantics — returns the Response', async () => {
    const withBlocker = defineGate<
      'blocker',
      undefined,
      Record<never, never>,
      Record<never, never>
    >({
      key: 'blocker',
      run: () => async () => new Response('nope', { status: 402 }),
    })

    const result = await withBlocker.run(undefined)(
      new Request('http://localhost/'),
      {},
    )
