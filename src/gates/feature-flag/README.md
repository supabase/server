# `@supabase/server/gates/feature-flag`
...
> This is the worked example for gate authors. The implementation is short and well-commented — read [`with-feature-flag.ts`](./with-feature-flag.ts) alongside the [authoring guide](../README.md) to see how each piece of `defineGate` lands in practice.
...
Place `withFeatureFlag` _after_ `withSupabase` to target by user identity:
...
The gate occupies `ctx.featureFlag` — only one `withFeatureFlag` can compose into a stack at a time. For multiple flags on the same route, write a single composite evaluator that returns a richer verdict, or run separate routes per flag.
